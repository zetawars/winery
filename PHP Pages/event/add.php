...
//	echo $_POST["txtBox"][1];
	...
	$myQuery = "INSERT INTO Event VALUES ('NULL','$subject')";
//	echo $myQuery;
...
if (mysqli_num_rows($columns) > 0) {
    while ($row = mysqli_fetch_assoc($columns)) {
		...
		else if($row['Field'] == "E_Date")
		{
			echo "<div><label for=\"ff-{$row['Field']}\" >{$row['Field']}</label><br />";
			echo "<input id=\"ff-{$row['Field']}\" name=\"txtBox[]\" type=\"date\" class=\"demoInputBox\" /></div>";			
		}
		else if($row['Field'] == "E_Time")
		{
			echo "<div><label for=\"ff-{$row['Field']}\" >{$row['Field']}</label><br />";
			echo "<input id=\"ff-{$row['Field']}\" name=\"txtBox[]\" type=\"time\" class=\"demoInputBox\" /></div>";			
		}
		else{
			echo "<div><label for=\"ff-{$row['Field']}\" >{$row['Field']}</label><br />";
