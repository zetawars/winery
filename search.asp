<%
' ----------------------------------------------------------------------------
' Zoom Search Engine 7.1 (6/Jan/2016)
' ASP search script
...
Dim MaxMatches, MaxContextSeeks, MaxSearchTime, FileTypePluginStart, PluginOpenNewWindow
Dim UseDomainDiversity, MaxDomainDiversity, div_res_count
Dim DefaultSort, DateRangeSearch, DateRangeFormat, DatePickerPath
Dim SearchWords, OrigSearchWords, RegExpSearchWords
Dim oldCodePage
...
Dim STR_RECOMMENDED, STR_CAT_SUMMARY, STR_SORTEDBY_DATE_ASC, STR_FORM_DATE_FROM, STR_FORM_DATE_TO, STR_FORM_DATE_BUTTON
...
Dim UseWildCards, SkippedOutputStr, SkippedWords, SkippedExactPhrase, UseAutoComplete
Dim sw_results, search_terms_ids, phrase_terms_ids, NumSearchWords
Dim catnames, NumCats, catpages, catindex, NumCatBytes
Dim IsZoomQuery, IsZoomAC, ZoomACOutput
...
Function GetUrlFromPageData(index)
	Dim pgdata
	pgdata = GetPageData(index)
	GetUrlFromPageData = pgdata(PAGEDATA_URL)
End Function
...
IsZoomAC = 0
if Request.QueryString("zoom_ac").Count <> 0 AND IsNumeric(Request.QueryString("zoom_ac")) then
	IsZoomAC = CInt(Request.QueryString("zoom_ac"))
	ZoomACOutput = ""
	if (IsZoomAC = 1) then
		SearchAsSubstring = 1
	end if
...
if Request.QueryString("zoom_sort").Count <> 0 AND IsNumeric(Request.QueryString("zoom_sort")) then
	...
	zoomsort = DefaultSort
end if

' date range
if (DateRangeSearch = 1) then
	date_from = ""
	date_to = ""
	if Request.QueryString("zoom_datefrom").Count <> 0 then
		date_from = Request.QueryString("zoom_datefrom")		
	end if
	if Request.QueryString("zoom_dateto").Count <> 0 then
		date_to = Request.QueryString("zoom_dateto")		
	end if	
end if
...
Dim OUTPUT_TAG_COUNT, OUTPUT_CATSUMMARY, OUTPUT_FORM_DATERANGE
...
TemplateShowTags = Array("<!--ZOOM_SHOW_FORMSTART-->", "<!--ZOOM_SHOW_FORMEND-->", "<!--ZOOM_SHOW_SEARCHBOX-->", "<!--ZOOM_SHOW_SEARCHBUTTON-->","<!--ZOOM_SHOW_RESULTSPERPAGE-->","<!--ZOOM_SHOW_MATCHOPTIONS-->","<!--ZOOM_SHOW_CATEGORIES-->","<!--ZOOM_SHOW_CUSTOMMETAOPTIONS-->","<!--ZOOM_SHOW_HEADING-->","<!--ZOOM_SHOW_SUMMARY-->","<!--ZOOM_SHOW_SUGGESTION-->","<!--ZOOM_SHOW_PAGESCOUNT-->","<!--ZOOM_SHOW_SORTING-->","<!--ZOOM_SHOW_SEARCHTIME-->","<!--ZOOM_SHOW_RECOMMENDED-->","<!--ZOOM_SHOW_PAGENUMBERS-->","<!--ZOOM_SHOW_CATSUMMARY-->","<!--ZOOM_SHOW_JAVASCRIPTS-->","<!--ZOOM_SHOW_DATERANGE-->")
...
Sub ShowDefaultForm()	
	...
	Response.Write(OutBuf(OUTPUT_FORM_DATERANGE))
	Response.Write(OutBuf(OUTPUT_FORM_END))
End Sub

Sub IncludeAdditionalJS()
	if (UseAutoComplete = 1) then
			Response.Write("<script language=""javascript"" type=""text/javascript"">") & VbCrLf
			Response.Write("var ZoomAutoComplete_URL = """ & selfURL & LinkBackJoinChar & "zoom_ac=1"";") & VbCrLf
			Response.Write(getMappedFileAsString(MapPath("zoom_autocomplete.js")))
			Dim onloadFunctionStr
			onloadFunctionStr = "function() { ZoomAutoComplete_OnLoad('zoom_searchbox');}"
			Response.Write("if (window.addEventListener) window.addEventListener('load', " & onloadFunctionStr & ", false);")
			Response.Write(" else if (window.attachEvent) window.attachEvent('onload', " & onloadFunctionStr & ");")
			Response.Write(" else window['onload'] = " & onloadFunctionStr) 
			Response.Write("</script>") & VbCrLf
			Response.Write("<style>") & VbCrLf
			Response.Write(getMappedFileAsString(MapPath("zoom_autocomplete.css")))
			Response.Write("</style>") & VbCrLf
	end if
	
	if (DateRangeSearch = 1) then
			Response.Write("<style>") & VbCrLf
			Response.Write(getMappedFileAsString(MapPath("zoom_datepicker.css")))
			Response.Write("</style>") & VbCrLf
			Response.Write("<script language=""javascript"" type=""text/javascript"">") & VbCrLf
			Response.Write(getMappedFileAsString(MapPath("zoom_datepicker.js")))
			Response.Write("</script>") & VbCrLf				
	end if	
End Sub
...
Sub ShowTemplate
	...
	dim IncludedJS, NeedJS
	tagPtr = ""
	IncludedJS = False
	
	NeedJS = False
	if (UseAutoComplete = 1 OR DateRangeSearch = 1) then
		NeedJS = True
	end if
	...
	do while fp_template.AtEndOfStream <> True
		...
		if (NeedJS = True AND IncludedJS = False AND InStr(templatePtr, "</head>") <> 0) then								
			IncludeAdditionalJS()			
			IncludedJS = True
		end if
		...
		elseif (InStr(tagPtr, TemplateDefaultTag) <> 0) then
			...
			if (NoCharset = 0) then		
				oldCodePage = Response.Codepage
				Response.Codepage = 1252	' this is necessary to not re-encode the already encoded dictionary words
			end if
			Response.Write(OutResBuf)
			if (NoCharset = 0) then
				Response.Codepage = oldCodePage
			end if
	...
	if (NeedJS = True AND IncludedJS = False) then			
		IncludeAdditionalJS()			
		IncludedJS = True
	end if		
End Sub
...
Sub SkipSearchWord(sw)
	if (SearchWords(sw) <> "") then
		...
		SkippedOutputStr = SkippedOutputStr & """<b>" & OrigSearchWords(sw) & "</b>"""
		SearchWords(sw) = ""
	end if
...
Sub ShellSortByDate(array, datetime, isAsc)
	dim first, last, num, distance, index, index2
	dim value, value0, value2, value3, value4, value5
	last = UBound(array, 2)
	first = 0	'LBound(array, 2)
	...
	do
		...
		for index = (distance + first) to last
			...
			index2 = index
			do while (index2 - distance => first)				
				if (isAsc = 0 AND datetime(array(0, index2 - distance)) > datetime(value0)) then
					exit do				
				elseif (isAsc = 1 AND datetime(array(0, index2 - distance)) < datetime(value0)) then
					exit do
				end if				
...
function RecLinkWordMatch(rec_word, rec_idx)
	...
	sw = 0
	IsFound = 0
	if (LastRecLinkIdx = rec_idx) then
		Exit Function
	end if
	...
	do while (sw <= NumSearchWords AND IsFound = 0)
		...
		if (bMatched) then
			...
			if (PdfHighlight = 1 AND IsNoKeywordQuery = False) then
				if (InStr(urlLink, ".pdf") <> False) then
					urlLink = urlLink & "#search=%22"&Replace(query, """", "")&"%22"
				end if
			end if
...
Function getMappedFileAsString(byVal strFilename)
  Const ForReading = 1  
  Dim ts
  if (zoomfso.FileExists(strFilename) = False) then
		Response.Write("<b>Zoom files missing error:</b> Zoom is missing: " & strFilename & "<br />")
		Response.End
	end if
  Set ts = zoomfso.OpenTextFile(strFilename, ForReading)
...
if (FormFormat > 0) then
	' Insert the form
	OutBuf(OUTPUT_FORM_SEARCHBOX) = OutBuf(OUTPUT_FORM_SEARCHBOX) & "<label for=""zoom_searchbox"">" & STR_FORM_SEARCHFOR & " </label>" & VbCrlf
	OutBuf(OUTPUT_FORM_SEARCHBOX) = OutBuf(OUTPUT_FORM_SEARCHBOX) & "<input type=""text"" name=""zoom_query"" size=""20"" value=""" & htmlspecialchars(query) & """ id=""zoom_searchbox"" class=""zoom_searchbox"" />" & VbCrlf
	...
	if (FormFormat = 2) then
		OutBuf(OUTPUT_FORM_RESULTSPERPAGE) = OutBuf(OUTPUT_FORM_RESULTSPERPAGE) & "<span class=""zoom_results_per_page""><label for=""zoom_per_page"">" & STR_FORM_RESULTS_PER_PAGE & "</label>" & VbCrlf
		OutBuf(OUTPUT_FORM_RESULTSPERPAGE) = OutBuf(OUTPUT_FORM_RESULTSPERPAGE) & "<select name=""zoom_per_page"" id=""zoom_per_page"">" & VbCrlf
		...
		if (UseCats = 1) then
			...
			OutBuf(OUTPUT_FORM_CATEGORIES) = OutBuf(OUTPUT_FORM_CATEGORIES) & "<label for=""zoom_cat"">" & STR_FORM_CATEGORY & "</label>" & VbCrlf
			if (SearchMultiCats = 1) then
				...
				OutBuf(OUTPUT_FORM_CATEGORIES) = OutBuf(OUTPUT_FORM_CATEGORIES) & " /><label for=""zoom_cat-1"">" & STR_FORM_CATEGORY_ALL & "</label></li>"
				for zoomit = 0 to NumCats-1
					...
					OutBuf(OUTPUT_FORM_CATEGORIES) = OutBuf(OUTPUT_FORM_CATEGORIES) & " /><label for=""zoom-cat" & zoomit & """>" & catnames(zoomit) & "</label></li>" & VbCrlf
				next
		...
		if (UseMetaFields = 1) then
			...
			Dim labelID
			for fieldnum = 0 to NumMetaFields-1
				labelID = "zoom_metaform_" & metafields(fieldnum)(METAFIELD_FORM)
				if (metafields(fieldnum)(METAFIELD_TYPE) = METAFIELD_TYPE_NUMERIC) then
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<label for=""" & labelID & """>" & metafields(fieldnum)(METAFIELD_FORM) & ": </label>" 
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<input type=""text"" id=""" & labelID & """ name=""" & metafields(fieldnum)(METAFIELD_NAME) & """ size=""20"" value=""" & htmlspecialchars(meta_query(fieldnum)) & """ class=""zoom_metaform_numeric"" />" & VbCrLf
				elseif (metafields(fieldnum)(METAFIELD_TYPE) = METAFIELD_TYPE_DROPDOWN) then
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<label for=""" & labelID & """>" & metafields(fieldnum)(METAFIELD_FORM) & ": </label>"
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<select id=""" & labelID & """ name=""" & metafields(fieldnum)(METAFIELD_NAME) & """ class=""zoom_metaform_dropdown"">" & VbCrLf
				...
				elseif (metafields(fieldnum)(METAFIELD_TYPE) = METAFIELD_TYPE_MULTI) then
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<label for=""" & labelID & """>" & metafields(fieldnum)(METAFIELD_FORM) & ": </label>" 
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<select multiple id=""" & labelID & """ name=""" & metafields(fieldnum)(METAFIELD_NAME) & "[]"" class=""zoom_metaform_dropdown"">" & VbCrLf
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<option value=""-1"">" & STR_FORM_CATEGORY_ALL & "</option>"
					ddi = 0
					For Each ddv in metafields(fieldnum)(METAFIELD_DROPDOWN)
						OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<option value=""" & ddi & """"
				...
				elseif (metafields(fieldnum)(METAFIELD_TYPE) = METAFIELD_TYPE_MONEY) then
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<label for=""" & labelID & """>" & metafields(fieldnum)(METAFIELD_FORM) & ": </label>"
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & MetaMoneyCurrency & "<input type=""text"" id=""" & labelID & """ name=""" & metafields(fieldnum)(METAFIELD_NAME) & """ size=""7"" value=""" & htmlspecialchars(meta_query(fieldnum)) & """ class=""zoom_metaform_money"" />" & VbCrLf
				else					
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<label for=""" & labelID & """>" & metafields(fieldnum)(METAFIELD_FORM) & ": </label>"
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<input type=""text"" id=""" & labelID & """ name=""" & metafields(fieldnum)(METAFIELD_NAME) & """ size=""20"" value=""" & htmlspecialchars(meta_query(fieldnum)) & """ class=""zoom_metaform_text"" />" & VbCrLf
				end if
			next
			OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "</span>" & VbCrLf
		end if
		OutBuf(OUTPUT_FORM_MATCH) = OutBuf(OUTPUT_FORM_MATCH) & "<span class=""zoom_match""><label for=""zoom_match"">" & STR_FORM_MATCH & "</label>" & VbCrlf
		if (andq = 0) then
			OutBuf(OUTPUT_FORM_MATCH) = OutBuf(OUTPUT_FORM_MATCH) & "<label for=""zoom_match_any""><input type=""radio"" id=""zoom_match_any"" name=""zoom_and"" value=""0"" checked=""checked"" />" & STR_FORM_ANY_SEARCH_WORDS & "</label>" & VbCrlf
			OutBuf(OUTPUT_FORM_MATCH) = OutBuf(OUTPUT_FORM_MATCH) & "<label for=""zoom_match_all""><input type=""radio"" id=""zoom_match_all"" name=""zoom_and"" value=""1"" />" & STR_FORM_ALL_SEARCH_WORDS & "</label>" & VbCrlf
		else
			OutBuf(OUTPUT_FORM_MATCH) = OutBuf(OUTPUT_FORM_MATCH) & "<label for=""zoom_match_any""><input type=""radio"" id=""zoom_match_any"" name=""zoom_and"" value=""0"" />" & STR_FORM_ANY_SEARCH_WORDS & "</label>" & VbCrlf
			OutBuf(OUTPUT_FORM_MATCH) = OutBuf(OUTPUT_FORM_MATCH) & "<label for=""zoom_match_all""><input type=""radio"" id=""zoom_match_all"" name=""zoom_and"" value=""1"" checked=""checked"" />" & STR_FORM_ALL_SEARCH_WORDS & "</label>" & VbCrlf
		end if
		...
		if (DateRangeSearch = 1) then
			OutBuf(OUTPUT_FORM_DATERANGE) = "<span class=""zoom_daterange"">" & VbCrLf
			Dim dateformatStr
			if (DateRangeFormat = 1) then
				dateformatStr = "mdy"
			else
				dateformatStr = "dmy"
			end if
			OutBuf(OUTPUT_FORM_DATERANGE) = OutBuf(OUTPUT_FORM_DATERANGE) & "<label for=""zoom_datefrom"">" & STR_FORM_DATE_FROM & " </label><input id=""zoom_datefrom"" name=""zoom_datefrom"" value=""" & date_from & """><input type=""button"" value=""" & STR_FORM_DATE_BUTTON & """ onclick=""displayDatePicker('zoom_datefrom', false, '" & dateformatStr & "');"" />"
			OutBuf(OUTPUT_FORM_DATERANGE) = OutBuf(OUTPUT_FORM_DATERANGE) & "<label for=""zoom_dateto"">" & STR_FORM_DATE_TO & " </label><input id=""zoom_dateto"" name=""zoom_dateto"" value=""" & date_to & """><input type=""button"" value=""" & STR_FORM_DATE_BUTTON & """ onclick=""displayDatePicker('zoom_dateto', false, '" & dateformatStr & "');"" />"
			OutBuf(OUTPUT_FORM_DATERANGE) = OutBuf(OUTPUT_FORM_DATERANGE) & "</span>"
		end if
	else
...
Dim NoSearch
NoSearch = 0
Dim IsNoKeywordQuery
IsNoKeywordQuery = False
if (Len(query) = 0 AND IsZoomAC = 0) then
	if (UseMetaFields = 1) then
...
if (NoSearch = 0) then
	...
	if (InStr(WordJoinChars, ":") = False) then
		...
		query = Replace(query, "site:", "[#@!]")
		query = Replace(query, "url:", "[#@!]")
		query = Replace(query, ":", " ")
		' We then restore it
		query = Replace(query, "[#@!]", "site:")
		query = Replace(query, "[#@!]", "url:")
	...
	dim quote_terms, term, exclude_terms, tmp_query
	quote_terms = Array()
	exclude_terms = Array()
	...
	for each term in quote_terms
		...
		SearchWords(zoomit) = Trim(Replace(term, """", ""))		
	next
	...
	if (IsZoomAC = 1) then
		Dim ac_count, ac_line
		acfile = split(ReadDatFile(zoomfso, "zoom_autocomplete.zdat"), VbCrLf)
		ac_count = UBound(acfile)
		for zoomit = 0 to ac_count-1
			ac_line = acfile(zoomit)
			for sw = 0 to NumSearchWords-1
				sw_len = Len(SearchWords(sw))				
				if (LCase(Left(ac_line, sw_len)) = LCase(SearchWords(sw))) then
					ZoomACOutput = ZoomACOutput & "<div class=""zoom_ac_item"" onclick=""ZoomAutoComplete_UseSuggestion('" & ac_line & "');""><a class=""zoom_ac_item"" href=""#"" target=""_self"">" & ac_line & "</a></div>" & VbCrLf
				end if
			next
		next
		Response.Write(ZoomACOutput)
		Response.End
	end if
	...
	for zoomit = 0 to pageinfo_count-1
		...
		filetype(zoomit) = GetBytes(bfp_pageinfo, 1)
	...
	Dim outputline, pagesCount, matches, relative_pos, current_pos
	...
	if (UseDomainDiversity = 1) then		
		Dim div_res_table()
		Dim div_res_urls()
		Redim preserve div_res_table(3)
		Redim preserve div_res_urls(3)
		div_res_count = 0
	end if
	...
	for sw = 0 to NumSearchWords-1
		...
		' initialize to zero here so we can tell if we've got one value already
		search_terms_ids(sw) = 0
		...
		if (bSkipped = False) then
			...
			' check if this is a search option (e.g. "site:mysite.com")
			if (InStr(SearchWords(sw), ":") <> 0) then
				if (Left(SearchWords(sw), 5) = "site:") then				
					searchUrlBuffer = Mid(SearchWords(sw), 6)
				elseif (Left(SearchWords(sw), 4) = "url:") then
					searchUrlBuffer = Mid(SearchWords(sw), 5)
				end if
				
				if (searchUrlBuffer <> "") then
					exclude_count = exclude_count + 1
					' if this site: term is the only search term in the query, then we enable 
					' empty query behaviour (so we return all results)
					if (NumSearchWords = 1) then
						IsNoKeywordQuery = true
						' we need to break out of the loop here and not continue searching through
						' the dictionary (and waste time) when there's no word to look up!
						' we'll continue to the top of the loop (where the while condition will break
						' since IsNoKeywordQuery is flagged)
						exit for
					end if
				end if
			end if
			...
			if (WordNotFound <> 1) then
				...
				for zoomit = 0 to dict_count
					...
					elseif (UseWildCards(sw) = 1) then
						Dim varWord
						Dim varCount
						varCount = dict(DICT_VARCOUNT, zoomit)
						if (varCount > 0) then
							for vi = 1 to varCount
								varWord = GetDictionaryWord(zoomit, vi)
								bMatched = regExp.Test(varWord)
								if (bMatched) then
									exit for
								end if
							next						
	...
	Dim dateRangeParams, UseDateRange
	Dim from_datetime, to_datetime
	dateRangeParams = ""
	from_datetime = ""
	to_datetime = ""
	UseDateRange = False
	if (DateRangeSearch = 1) then
		if (Len(date_from) > 0 AND Len(date_to) > 0) then
			Dim tmpFromDate, tmpToDate, fromArray, toArray
			fromArray = Split(date_from, "/")
			toArray = Split(date_to, "/")
			if (UBound(fromArray) = 2 AND UBound(toArray) = 2) then
				if (IsNumeric(fromArray(0)) AND IsNumeric(fromArray(1)) AND IsNumeric(fromArray(2)) AND IsNumeric(toArray(0)) AND IsNumeric(toArray(1)) AND IsNumeric(toArray(2))) then
					if (DateRangeFormat = 1) then
						tmpFromDate = DateSerial(Int(fromArray(2)), Int(fromArray(0)), Int(fromArray(1)))
						tmpToDate = DateSerial(Int(toArray(2)), Int(toArray(0)), Int(toArray(1)))
					else
						tmpFromDate = DateSerial(Int(fromArray(2)), Int(fromArray(1)), Int(fromArray(0)))
						tmpToDate = DateSerial(Int(toArray(2)), Int(toArray(1)), Int(toArray(0)))
					end if
					from_datetime = UDate(tmpFromDate)
					' add time to tmpToDate to end of day
					tmpToDate = DateAdd("h", 24, tmpToDate)
					to_datetime = UDate(tmpToDate)	
					if (from_datetime >= 0 AND to_datetime > 0) then
						UseDateRange = True
						dateRangeParams = "&amp;zoom_datefrom=" & Server.URLEncode(date_from) & "&amp;zoom_dateto=" & Server.URLEncode(date_to)
					end if
				end if
			end if			
		end if
	...
	for zoomit = 0 to pagesCount-1
		...
		if (pgdataoffset(zoomit) = 0) then	'filter out if deleted
			IsFiltered = True
		end if
		
		if (res_table(0, zoomit) > 0 OR IsNoKeywordQuery = True) then

			if (IsFiltered = False AND searchUrlBuffer <> "") then
				pageUrlBuffer = GetUrlFromPageData(zoomit)
				if (InStr(LCase(pageUrlBuffer), LCase(searchUrlBuffer)) = False) then
					IsFiltered = True
				elseif (IsNoKeywordQuery = True) then
			...
			if (UseMetaFields = 1 AND IsFiltered = False) then
				...
				for fieldnum = 0 to NumMetaFields-1
					...
					if (IsNoKeywordQuery = True AND IsFiltered = False) then
						res_table(0, zoomit) = Int(res_table(0, zoomit)) + 1
						res_table(1, zoomit) = Int(res_table(1, zoomit)) + 1
					end if
			...
			if (IsFiltered = False AND DateRangeSearch = 1 AND UseDateRange = True) then
				if (datetime(zoomit) < from_datetime) then
					IsFiltered = True
				elseif (datetime(zoomit) > to_datetime) then
					IsFiltered = True
				end if
			end if
	...
	' Sort the results
	if (matches > 1) then
		if (UseDateTime = 1 AND zoomsort = 1) then
				call ShellSortByDate(output, datetime, 0)
		elseif (UseDateTime = 1 AND zoomsort = 2) then
				call ShellSortByDate(output, datetime, 1)			
		else
	...
	if (UseDomainDiversity = 1) then
		if (zoompage = 1 AND matches > 5) then
			arrayline = 0
			div_res_count = 0
			Dim domainBuffer, IsDifferentDomain
			do while (arrayline < matches AND div_res_count < MaxDomainDiversity)
				pageUrlBuffer = GetUrlFromPageData(output(0, arrayline))
				domainBuffer = split(pageUrlBuffer, "/")
				domainBuffer = domainBuffer(2)				
				IsDifferentDomain = true
				for div_url_i = 0 to div_res_count
					if (Lcase(div_res_urls(div_url_i)) = Lcase(domainBuffer)) then						
						IsDifferentDomain = false						
						exit for
					end if					
				next
				if (IsDifferentDomain = true) then
					div_res_table(div_res_count) = arrayline
					div_res_urls(div_res_count) = domainBuffer
					div_res_count = div_res_count + 1
				end if				
	...
	if (matches < 3) then
		if (andq = 1 AND NumSearchWords > 1) then
			OutBuf(OUTPUT_SUMMARY) = OutBuf(OUTPUT_SUMMARY) & "<div class=""suggestion""><br />" & STR_POSSIBLY_GET_MORE_RESULTS & " <a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=0" & "&amp;zoom_sort=" & zoomsort & """>" & STR_ANY_OF_TERMS & "</a>.</div>"
		elseif (UseCats = 1 AND zoomcat(0) <> -1) then
			OutBuf(OUTPUT_SUMMARY) = OutBuf(OUTPUT_SUMMARY) & "<div class=""suggestion""><br />" & STR_POSSIBLY_GET_MORE_RESULTS & " <a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_per_page=" & per_page & "&amp;zoom_cat=-1" & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=" & zoomsort & """>" & STR_ALL_CATS & "</a>.</div>"
		end if
	end if
		...
		for catit = 0 to NumCats-1			
			...
			if (CatCounter(catit) > 0) then
				if (CatCounter(catit) <> matches) then
					...
					OutBuf(OUTPUT_CATSUMMARY) = OutBuf(OUTPUT_CATSUMMARY) & "<li><a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_cat[]=" & catit & dateRangeParams & "&amp;zoom_per_page=" & per_page & "&amp;zoom_and=" & andq & "&amp;zoom_sort=" & zoomsort & """>" & catnames(catit)
	...
	if (Spelling = 1) then
		...
		if (SuggestionsCount > 0) then
			OutBuf(OUTPUT_SUGGESTION) = OutBuf(OUTPUT_SUGGESTION) & "<div class=""suggestion""><br />" & STR_DIDYOUMEAN & " <a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & Server.URLEncode(SuggestStr) & metaParams & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=0&amp;zoom_sort=" & zoomsort & """>" & SuggestStr & "</a>"
			if (Len(word2) > 0) then
				OutBuf(OUTPUT_SUGGESTION) = OutBuf(OUTPUT_SUGGESTION) & " " & STR_OR & " <a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & Server.URLEncode(word2) & metaParams & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=" & zoomsort & """>" & word2 & "</a>"
			end if
			if (Len(word3) > 0) then
				OutBuf(OUTPUT_SUGGESTION) = OutBuf(OUTPUT_SUGGESTION) & " " & STR_OR & " <a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & Server.URLEncode(word3) & metaParams & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=" & zoomsort & """>" & word3 & "</a>"
			end if
	...
	if (matches > 1 AND UseDateTime = 1) then
		...
		if (zoomsort = 1) then
			OutBuf(OUTPUT_SORTING) = OutBuf(OUTPUT_SORTING) & "<a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_page=" & zoompage & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=0"">" & STR_SORTBY_RELEVANCE & "</a> / <a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_page=" & zoompage & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=2"">" & STR_SORTEDBY_DATE & "</a>"
		elseif (zoomsort = 2) then
			OutBuf(OUTPUT_SORTING) = OutBuf(OUTPUT_SORTING) & "<a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_page=" & zoompage & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=0"">" & STR_SORTBY_RELEVANCE & "</a> / <a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_page=" & zoompage & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=1"">" & STR_SORTEDBY_DATE_ASC & "</a>"
		else			
			OutBuf(OUTPUT_SORTING) = OutBuf(OUTPUT_SORTING) & "<b>" & STR_SORTEDBY_RELEVANCE & "</b> / <a href=""" & SelfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_page=" & zoompage & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=1"">" & STR_SORTBY_DATE & "</a>"
		end if
		OutBuf(OUTPUT_SORTING) = OutBuf(OUTPUT_SORTING) & "</div>"
	end if

	Dim result_limit, urlLink, display_num
	...
	do while (arrayline < matches AND arrayline < result_limit)
		...
		if (UseDomainDiversity = 1 AND arrayline < div_res_count) then
			ipage = output(div_res_table(0,arrayline))
			score = output(div_res_table(1, arrayline))
			termsmatched = output(div_res_table(2, arrayline))
			output(div_res_table(1, arrayline)) = -1
		else					
			ipage = output(0, arrayline)
			score = output(1, arrayline)
			termsmatched = output(2, arrayline)
		...
		if (PluginOpenNewWindow = 1 AND filetype(ipage) >= FileTypePluginStart) then
		...
		if (DisplayNumber = 1) then
			OutResBuf = OutResBuf & "<b>" & (display_num) & ".</b>&nbsp;"
		end if
		...
		if (DisplayContext = 1 AND output(2, arrayline) > 0 AND IsNoKeywordQuery = False) then
				...
				for cti = 0 to context_word_count
					if (word_id = 0 OR word_id = 1 OR word_id > dict_count) then
						...
						if (word_id >= NumKeywords) then
							...
							if (Highlighting = 1 AND IsNoKeywordQuery = False AND (startpos+last_bytesread) = origpos) then
		...
		if (DisplayTerms = 1) then
			info_str = info_str & STR_RESULT_TERMS_MATCHED & " " & termsmatched
		end if
	...
	if (num_pages > 1) then
		...
		if (zoompage > 1) then
			OutBuf(OUTPUT_PAGENUMBERS) = OutBuf(OUTPUT_PAGENUMBERS) & "<a href=""" & selfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_page=" & (zoompage-1) & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=" & zoomsort & """>&lt;&lt; " & STR_RESULT_PAGES_PREVIOUS & "</a> "
		end if
		...
		for zoomit = start_range to end_range
			if (Int(zoomit) = Int(zoompage)) then
				...
				OutBuf(OUTPUT_PAGENUMBERS) = OutBuf(OUTPUT_PAGENUMBERS) & "<a href=""" & selfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_page=" & zoomit & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=" & zoomsort & """>" & zoomit & "</a> "
			end if
		next
		if (Int(zoompage) <> Int(num_pages)) then
			OutBuf(OUTPUT_PAGENUMBERS) = OutBuf(OUTPUT_PAGENUMBERS) & "<a href=""" & selfURL & LinkBackJoinChar & "zoom_query=" & queryForURL & metaParams & "&amp;zoom_page=" & (zoompage+1) & "&amp;zoom_per_page=" & per_page & query_zoom_cats & dateRangeParams & "&amp;zoom_and=" & andq & "&amp;zoom_sort=" & zoomsort & """>" & STR_RESULT_PAGES_NEXT & " &gt;&gt;</a> "
		end if
...
if (ZoomInfo = 1) then
	OutBuf(OUTPUT_PAGENUMBERS) = OutBuf(OUTPUT_PAGENUMBERS) & VbCrLf & "<p style=""text-align: center;""><small>" & STR_POWEREDBY & " <a href=""http://www.wrensoft.com/zoom/"" target=""_blank""><b>Zoom Search Engine</b></a></small></p>"
end if
